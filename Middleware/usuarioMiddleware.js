...
const validarDatos = [
  ...
  check("email")
    .isEmail()
    .withMessage("El email debe tener la estructura correcta de un correo"),
  (req, res, next) => {
...
const numeroSolicitudes = (req, res, next) => {
  contador++;
  console.log(
    `Nueva Solicitud: [${new Date().toISOString()}] ${req.method} ${
      req.originalUrl
    }`
  );
...
const autenticar = (req, res, next) => {
  try {
    ...
    if (!authHeader) {
      return res
        .status(401)
        .json({ mensaje: "Acceso denegado, token requerido" });
    }
